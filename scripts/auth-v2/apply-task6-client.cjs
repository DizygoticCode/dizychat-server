...
if (login.includes('const ADMIN_USERNAMES')) {
  const start = login.indexOf('  <script>\n    const ADMIN_USERNAMES');
  if (start === -1) throw new Error('Task 6 legacy admin browser script start not found');
  const endMarker = '  </script>';
  const end = login.indexOf(endMarker, start);
  if (end === -1) throw new Error('Task 6 legacy admin browser script end not found');
  login = `${login.slice(0, start)}${login.slice(end + endMarker.length)}`;
}

// ---------------- Socket/session bootstrap ----------------
if (!chat.includes('const DIZYCHAT_ACCOUNT_SESSION_KEY = "dizychat-account-session-v2";')) {
  const socketStart = 'const { url: socketUrl, options: socketOptions } = resolveSocketConfig();';
  const socketEnd = 'window.socket = socket;';
  chat = replaceBetween(
    chat,
...
function syncAccountUi() {
  ...
  if (accountIdentity) {
    ...
    accountIdentity.textContent = identity ? `${identity.username} · ${String(identity.role || "user").toUpperCase()}` : "";
  }
...
function applyAccountSession(session) {
  ...
  const identity = session?.identity && typeof session.identity === "object" ? session.identity : null;
  accountState.sessionToken = token;
  accountState.identity = identity;
...
// ---------------- URL sanitisation ----------------
if (chat.includes('const prefillPassword = urlParams.get("password") || "";')) {
  chat = replaceOnce(
    chat,
    'Task 6 legacy password query read',
...
if (chat.includes('if (prefillPassword) {')) {
  const start = chat.indexOf('if (prefillPassword) {');
  const endMarker = 'if (usernamePlaceholder && usernameInput) {';
  const end = chat.indexOf(endMarker, start);
  if (end === -1) throw new Error('Task 6 legacy password prefill end not found');
  const replacement = `if (prefillRoom) {
  updateQueryParams(prefillRoom);
}

`;
  const existingRoomStart = chat.lastIndexOf('if (prefillRoom) {', start);
  if (existingRoomStart === -1) throw new Error('Task 6 room prefill start not found');
  chat = `${chat.slice(0, existingRoomStart)}${`if (prefillRoom) {
  lastRoomName = prefillRoom;
  if (roomInput) roomInput.value = prefillRoom;
}

${replacement}`}${chat.slice(end)}`;
}

if (chat.includes('function updateQueryParams(room, password) {')) {
  const startMarker = 'function updateQueryParams(room, password) {';
  const endMarker = '\n}\n\nfunction showLanding';
  const start = chat.indexOf(startMarker);
  const end = chat.indexOf(endMarker, start);
  if (start === -1 || end === -1) throw new Error('Task 6 URL helper seam not found');
  const replacement = `function updateQueryParams(room) {
  try {
  ...
  chat = `${chat.slice(0, start)}${replacement}${chat.slice(end + 2)}`;
}
...
if (chat.includes('[usernameInput, roomInput, passwordInput, adminPasswordInput]')) {
  chat = chat.replace(
...
// ---------------- Account restore/reconnect ----------------
if (chat.includes('// If we were previously in a room, automatically rejoin it after reconnecting.')) {
  const start = chat.indexOf('socket.on("connect", () => {');
  const end = chat.indexOf('socket.on("disconnect", () => {', start);
  if (start === -1 || end === -1) throw new Error('Task 6 reconnect seam not found');
  const replacement = `socket.on("connect", () => {
...
// ---------------- Role-backed moderation UI ----------------
chat = chat.replaceAll('appState.isAdmin = false;', 'appState.isAdmin = accountRoleCanModerate();');
if (chat.includes('socket.on("admin status", ({ isAdmin }) => {')) {
  const start = chat.indexOf('socket.on("admin status", ({ isAdmin }) => {');
  const endMarker = '\n});\n\n// ------------------- File Upload Helpers';
  const end = chat.indexOf(endMarker, start);
  if (end === -1) throw new Error('Task 6 admin status listener end not found');
  chat = `${chat.slice(0, start)}// Account roles are restored from Auth v2 session metadata; no admin-password status event.${chat.slice(end + '\n});'.length)}`;
}

// ---------------- Logout ----------------
if (!chat.includes('accountLogoutBtn.addEventListener("click"')) {
  const marker = 'if (copyJoinLinkBtn) {';
  const idx = chat.indexOf(marker);
  if (idx === -1) throw new Error('Task 6 logout insertion seam not found');
  const logoutBlock = `if (accountLogoutBtn) {
...
`;
  chat = `${chat.slice(0, idx)}${logoutBlock}${chat.slice(idx)}`;
}
...
// ---------------- Chromium smoke follows guest mode ----------------
uiTest = uiTest.replaceAll("await page.waitForSelector('#join-btn', { timeout: 30000 });", "await page.waitForSelector('#guest-join-btn', { timeout: 30000 });");
uiTest = uiTest.replaceAll("await page.fill('#username-input', 'TesterBot');", "await page.fill('#guest-username', 'TesterBot');");
uiTest = uiTest.replaceAll("await page.click('#join-btn');", "await page.click('#guest-join-btn');");

// ---------------- Final guards ----------------
const forbiddenLogin = ['id="admin-password"', 'ADMIN_USERNAMES', 'id="username-input"', 'id="join-btn"'];
for (const token of forbiddenLogin) {
  if (login.includes(token)) throw new Error(`Task 6 forbidden legacy landing token remains: ${token}`);
}
const forbiddenChat = ['adminPasswordInput', '"admin auth"', "'admin auth'", '.get("password")', ".get('password')", 'params.set("password"', "params.set('password'"];
for (const token of forbiddenChat) {
...
for (const token of requiredChat) {
