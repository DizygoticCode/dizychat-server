...
function replaceOnce(needle, replacement, label) {
  ...
  if (source.indexOf(needle, first + needle.length) >= 0) throw new Error(`duplicate anchor: ${label}`);
...
replaceOnce("// @version      1.12.3", "// @version      1.12.4", "version");
...
`    function safeBurnMemorySnippet(value, max = 82) {
    ...
    function buildBurnMemoryContext(ctx, profile) {
        ...
        const addEvidence = (item) => {
            ...
            evidence.push(Object.freeze({
                id: String(item.id || simpleCuratedHash(item.text)).slice(0, 120),
                ...
                text: safeBurnMemorySnippet(item.text),
        ...
        if (!incomingBlocked && profile) {
            ...
            for (let i = 0; i < recent.length; i += 1) {
                ...
                for (let j = i + 1; j < recent.length; j += 1) {
                    ...
                    if (overlap >= 2 && a.negated !== b.negated && a.text !== b.text) {
                        const relevance = Math.max(overlapCount(currentTokens, a.tokens || []), overlapCount(currentTokens, b.tokens || []));
                        if (relevance >= 1) addEvidence({ id: `flip:${a.seq}:${b.seq}`, source: "history", kind: "contradiction", family: "contradiction", text: relevance === overlapCount(currentTokens, b.tokens || []) ? b.text : a.text, rank: 88 + relevance, burnId: `flip:${Math.min(a.seq, b.seq)}:${Math.max(a.seq, b.seq)}` });
                    }
                }
            }
    ...
    function chooseBurnMemoryAngle(memoryContext) {
        ...
        const candidates = [];
        const recentAngles = curatedBurnStore.recentAngleIds || [];
        const recentFamilies = curatedBurnStore.recentMemoryFamilies || [];
        const add = (id, source, family, rank, evidence = null, seed = null) => {
            const anglePenalty = recentAngles.slice(-8).filter((value) => value === id).length * 9;
            const familyPenalty = recentFamilies.slice(-8).filter((value) => value === family).length * 5;
            candidates.push({ id, source, family, evidence, seed, rank: rank - anglePenalty - familyPenalty });
        };
        ...
        const relevantCallback = (memoryContext.evidence || []).find((item) => item.kind === "callback");
        if (relevantCallback) add("attack_reversal", relevantCallback.source, relevantCallback.family || "callback", relevantCallback.rank + 6, relevantCallback);

        const text = memoryContext.currentText || "";
        if (/\b(?:best|better than|smartest|genius|expert|always right|never wrong|easy win|destroyed|owned|rekt)\b/i.test(text)) add("brag_deflation", "live", "bragging", 63, relevantCallback);
        if (/\b(?:idiot|moron|stupid|clown|loser|pathetic|dumb|muppet|bellend|wanker|tosser)\b/i.test(text)) add("attack_reversal", "live", "direct_attack", 60, relevantCallback);
        if (/\?|\b(?:why|what|how|prove it|answer me|come on|try harder|is that it)\b/i.test(text)) add("bait_deflection", "live", "bait", 54, relevantCallback);
        ...
        add("generic_banter", "live", memoryContext.contextRanking?.[0]?.family || "generic_savage", 28, null, null);
    ...
    function composeMemoryBurn(memoryContext, angle, attempt = 0) {
        ...
        const variants = {
            contradiction_callback: [
                ...
                callback ? `@${target} the callback is “${callback}”. Awkward time to launch the opposite version.` : `@${target} your old claim and new claim are currently fighting over custody of the truth.`
            ],
            repeat_callback: [
    ...
    function rememberMemoryBurn(candidate, angle) {
        ...
        curatedBurnStore.lastMemoryDiagnostic = {
            source: String(angle.source || "local").slice(0, 24),
            angle: String(angle.id || "generic_banter").slice(0, 40),
            family: String(angle.family || "generic_savage").slice(0, 40),
    ...
    function selectCuratedBurnWithOptions(ctx, options = {}) {
        ...
        const buildProfile = profileReady ? profile : null;
        const memoryContext = buildBurnMemoryContext(ctx, buildProfile);
        ...
        for (let attempt = 0; attempt < 4; attempt += 1) {
            ...
            if (attempt === 2 && angle.id !== "generic_banter") {
                const fallbackContext = Object.freeze({ ...memoryContext, evidence: Object.freeze([]) });
                angle = chooseBurnMemoryAngle(fallbackContext) || angle;
            }
        }
        ...
        pendingCuratedBurnSelection = {
            kind: seed ? "seed" : evidence?.source === "live" ? "live" : evidence?.source === "history" ? "history" : "live",
            username,
...
`        const custom = window.rumbleBlocker?.customBurnGenerator;
        ...
        const customMemoryContext = Object.freeze({
            ...
            evidence: Object.freeze((customRawMemoryContext.evidence || []).slice(0, 4).map((item) => Object.freeze({
                source: item.source,
                kind: item.kind,
                family: item.family,
                text: safeBurnMemorySnippet(item.text, 72),
                count: item.count
            })))
        });
...
console.log("Applied burn memory intelligence patch");
