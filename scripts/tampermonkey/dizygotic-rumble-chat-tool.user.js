...
(function () {
            ...
            .forEach((el) => {
                ...
                if (settings.showTimestamps && usernameEl) {
                    if (!el._timestampApplied) {
                        ...
                        const parent = usernameEl.parentElement || el;
                        parent.insertBefore(timeNode, usernameEl.nextSibling);

                        el._timestampNode = timeNode;
                        el._timestampApplied = true;
                    } else if (el._timestampNode && el._timestampNode.textContent) {
                        el._timestampNode.textContent = formatTime(new Date());
                    }
                ...
                let maskedHTML = null;
                if (shouldMask) {
                    const maskSignature = settings.blockedKeywords.filter(Boolean).join("|");
                    const originalSource = el._originalMessage || "";
                    if (
                        !el._maskedHTML ||
                        el._maskedSource !== originalSource ||
                        el._maskSignature !== maskSignature
                    ) {
                        ...
                        settings.blockedKeywords
                            ...
                            .forEach((kw) => {
                                ...
                                html = html.replace(re, (match) => "•".repeat(match.length));
                            });
                        el._maskedHTML = html;
                        el._maskedSource = originalSource;
                        el._maskSignature = maskSignature;
                    }
                    maskedHTML = el._maskedHTML;
                } else {
                    el._maskedHTML = null;
                    el._maskedSource = null;
                    el._maskSignature = null;
                }
                    ...
                    if (shouldMask && !shouldHide) {
                        ...
                        setTimeout(() => {
                            if (el._revealed) {
                                ...
                                msgEl.innerHTML = el._maskedHTML || maskedHTML;
                                msgEl.style.opacity = "0.95";
                                msgEl.style.color = settings.darkMode ? "#e0e0e0" : "#222";
                                msgEl.title = snippet;
                            }
    ...
    function openDirectMessage(targetDisplayName) {
        const myNickname = ((detectMyNickname(true) || "Guest") + "")
            .replace(/[\r\n]+/g, " ")
            .replace(/\s+/g, " ")
            .trim() || "Guest";
        let normalizedTarget = (targetDisplayName || "")
            .replace(/[\r\n]+/g, " ")
            .replace(/\s+/g, " ")
            .trim();
        ...
        if (!normalizedTarget) {
            normalizedTarget =
                prompt("Enter the username you want to direct message:", "")?.trim() || "";
        }
        ...
        const roomSuffix = normalizedTarget.toLowerCase().endsWith("s") ? "' Room" : "'s Room";
        const roomName = `${normalizedTarget}${roomSuffix}`;
