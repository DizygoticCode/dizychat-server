...
When users choose **Music mode On**, the token endpoint marks the call as music mode and returns high-fidelity audio settings to the browser. The browser then requests a stereo microphone track with echo cancellation, noise suppression, and automatic gain control disabled, and publishes it with a 510 kbps LiveKit/Opus target, DTX disabled, and RED disabled. This is an application-side publish setting, not a separate DizyChat media backend; actual quality can still vary with the user's microphone, browser, operating-system audio path, and network conditions. LiveKit Cloud's free Build plan has usage quotas and included allowances, but LiveKit documents hi-fi audio publish settings up to 510 kbps stereo rather than a free-plan-specific low-bitrate cap.
