...
shareBtn.addEventListener('click', () => {
  const roomPath = window.location.pathname.split("/room/")[1];
  const currentRoom = roomPath || room;
  let urlParams = new URLSearchParams(window.location.search);
  let currentUser = urlParams.get("nickname") || username;

  if (!currentUser) {
    currentUser = prompt("Enter your username to share this room:", "Guest") || "Guest";
  }

  const fullURL = `${window.location.origin}/room/${encodeURIComponent(currentRoom)}?nickname=${encodeURIComponent(currentUser)}`;
